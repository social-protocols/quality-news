...
Here is the current hacker news ranking formula. The problem is that it only considers 1) **upvotes** and 2) **age**. It doesn't consider 3) **timing** and 4)  **rank**. 

     rankingScore = pow(upvotes, 0.8) / pow(ageHours + 2, 1.8)

So a story that receives 100 upvotes at rank 1 at midday US time has the same ranking score as one that receives the same number of upvotes at rank 50 at midnight. 
...
Of course we should ask if stories at rank 1 get more upvotes **because** they are shown at rank 1, or are the stories shown at rank 1 **because** they get more upvotes? We need to untangle the causal effect of rank on upvotes from the effect of the algorithm. But what we've found is that 
[Todo: link to writeup] 

Once we do this, we can calculate the expected average upvote *share* at each rank. Here's the results for the HN front page:
...
So for example, roughly 10.2% [TODO: update this} of sitewide upvotes go to the first story on the home page, so `upvoteShare[1] = 10.2%`. The total share across all ranks adds up to 1.
...
We assume that each story has some "true" upvote rate, which is how much more or less likely users are to upvote that story than the average story. During each time interval, the number of upvotes is random, but the expected value (long term average) will be the upvote rate times the attention it has received.

    upvotes[time] ≈ upvoteRate * attention[time, rank[time]]

This relationship `upvotes ≈ upvoteRate * attention` holds even in the aggregate, regardless of the ranks at which upvotes actually occurred.
